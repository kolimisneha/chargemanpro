...
/* harmony import */ var _home_snehakolimi_workspaces_chargemanpro_node_modules_ngtools_webpack_src_loaders_direct_resource_js_charge_start_stop_page_html__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! !./node_modules/@ngtools/webpack/src/loaders/direct-resource.js!./charge-start-stop.page.html */ 93249);
...
/* harmony import */ var rxjs__WEBPACK_IMPORTED_MODULE_14__ = __webpack_require__(/*! rxjs */ 18252);
/* harmony import */ var rxjs_operators__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! rxjs/operators */ 88377);
/* harmony import */ var rxjs_operators__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! rxjs/operators */ 90758);
/* harmony import */ var rxjs_operators__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! rxjs/operators */ 88986);
/* harmony import */ var rxjs_operators__WEBPACK_IMPORTED_MODULE_13__ = __webpack_require__(/*! rxjs/operators */ 10592);
/* harmony import */ var rxjs_operators__WEBPACK_IMPORTED_MODULE_15__ = __webpack_require__(/*! rxjs/operators */ 94058);
...
let ChargeStartStopPage = class ChargeStartStopPage {
    constructor(navCtrl, modalController, A_route, router, utils, chargeReq) {
        ...
        this.guns = [];
        this.isLoadingGuns = false;
        this.connectorDetails = []; // Full connector objects from API
    }
    ngOnInit() {
        var _a, _b, _c;
        return (0,tslib__WEBPACK_IMPORTED_MODULE_8__.__awaiter)(this, void 0, void 0, function* () {
            ...
            this.loadConnectors(this.pageParams.deviceid);
            ...
            if (parseInt((_c = this.pageParams) === null || _c === void 0 ? void 0 : _c.chargeCount) > 0) {
                ...
                this.chargeReq.postRequestDetails(_constants__WEBPACK_IMPORTED_MODULE_2__.RELATIVE_URLS.CHARGING_SUMMARY, transaction_details).subscribe((res) => (0,tslib__WEBPACK_IMPORTED_MODULE_8__.__awaiter)(this, void 0, void 0, function* () {
                    var _d;
                    let selectedDuration = (_d = (yield this.utils.getStoredDetails(_constants__WEBPACK_IMPORTED_MODULE_2__.KEYS.SELECTED_DURATION))) === null || _d === void 0 ? void 0 : _d.value;
                    if (selectedDuration) {
    ...
    selectGun(gun) {
        ...
        // Update the active deviceid to the child connector's deviceid
        // so startCharging DB call and OCPP command use the correct connector
        const connector = this.connectorDetails.find((c) => parseInt(c.connectorid, 10) === gun);
        if (connector && connector.deviceid) {
            this.pageParams.deviceid = connector.deviceid;
            this.deviceProtocolDetails.deviceId = connector.deviceid;
            console.log('Selected gun', gun, '→ deviceid:', connector.deviceid);
        }
    }
    startCharging() {
        this.isChargeStarted = true;
    ...
    /**
     * Loads the available connectors/guns for this station from the server.
     * The parent device ID is derived by stripping the connector suffix
     * (e.g., 'cm2300typo-1' → 'cm2300typo').
     */
    loadConnectors(deviceId) {
        this.isLoadingGuns = true;
        // Strip the connector suffix to get the parent device ID
        const parentDeviceId = deviceId.includes('-')
            ? deviceId.split('-').slice(0, -1).join('-')
            : deviceId;
        const body = { deviceid: parentDeviceId };
        this.chargeReq.postRequestDetails(_constants__WEBPACK_IMPORTED_MODULE_2__.RELATIVE_URLS.GET_CONNECTORS_BY_PARENT, body).subscribe({
            next: (res) => {
                this.isLoadingGuns = false;
                if (res && res.length > 0) {
                    // Exclude the parent device record (where deviceid === deviceparent)
                    // Only keep child connectors (e.g. cmdc120nnd-1, cmdc120nnd-2)
                    const childConnectors = res.filter((c) => c.deviceid !== c.deviceparent);
                    const source = childConnectors.length > 0 ? childConnectors : res;
                    // Store full objects for use in selectGun
                    this.connectorDetails = source;
                    this.guns = source
                        .map((c) => parseInt(c.connectorid, 10))
                        .filter((n) => !isNaN(n))
                        .sort((a, b) => a - b);
                    console.log('Loaded connectors from API:', this.guns);
                }
                else {
                    // API returned empty — fall back to default 4 guns
                    console.warn('No connectors returned from API, falling back to default [1,2,3,4]');
                    this.guns = [1, 2, 3, 4];
                }
            },
            error: (err) => {
    ...
    stopCharging(type) {
        ...
        return (0,tslib__WEBPACK_IMPORTED_MODULE_8__.__awaiter)(this, void 0, void 0, function* () {
            ...
            this.chargeReq.postRequestDetails(relativeUrl, stop_body).pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_9__.map)((res) => {
                ...
                if (res && res.status && (res.status.toLowerCase() === 'accepted' || res.status.toLowerCase() === 'success')) {
                    if (deviceType.toLowerCase() === 'ocpp') {
                        device_command_body = {
                            "transactionId": parseInt(transactionID.transactionid || this.pageParams.transactionid, 10)
                        };
                    }
                    else {
                        device_command_body = {
            ...
            }), (0,rxjs_operators__WEBPACK_IMPORTED_MODULE_10__.mergeMap)((stopChargeRes) => {
                ...
                next: (res) => (0,tslib__WEBPACK_IMPORTED_MODULE_8__.__awaiter)(this, void 0, void 0, function* () {
                    var _e, _f;
            ...
            if (parseFloat((_c = this.walletBalance) === null || _c === void 0 ? void 0 : _c.value) > 0 || this.isSelfCharge) {
                ...
                if (user_sel == 1) {
                    ...
                    const start_body = {
                        ...
                        "deviceid": this.pageParams.deviceid,
                        "chargedate": moment__WEBPACK_IMPORTED_MODULE_6__().format('YYYY-MM-DD'),
                        "starttime": moment__WEBPACK_IMPORTED_MODULE_6__().format('YYYY-MM-DD HH:mm:ss'),
                    ...
                    this.chargeReq.postRequestDetails(_constants__WEBPACK_IMPORTED_MODULE_2__.RELATIVE_URLS.START_CHARGE, start_body).pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_9__.map)((res) => {
                        ...
                        return device_command_body;
                    }), (0,rxjs_operators__WEBPACK_IMPORTED_MODULE_10__.mergeMap)((startChargeRes) => {
    ...
    startChargeInterval(response) {
        ...
        this.chargingTimer = (0,rxjs__WEBPACK_IMPORTED_MODULE_11__.interval)(10000).pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_12__.exhaustMap)(() => this.chargeReq.postRequestDetails(_constants__WEBPACK_IMPORTED_MODULE_2__.RELATIVE_URLS.CHECK_CHARGE, interval_body).pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_13__.catchError)(err => {
            console.error('Charge interval error:', err);
            return (0,rxjs__WEBPACK_IMPORTED_MODULE_14__.of)(null);
        }))), (0,rxjs_operators__WEBPACK_IMPORTED_MODULE_15__.takeUntil)(this.destroy$)).subscribe((res) => {
            var _a;
            ...
            if (!this.isDurationSkipped && !this.isAutoCharge && this.timerVal > 0) {
                const minuteago = (res[0].minuteago || '00:00:00').split('.')[0];
                const parts = minuteago.split(':');
                let serverElapsedSec = 0;
                if (parts.length === 3) {
                    serverElapsedSec = (parseInt(parts[0], 10) || 0) * 3600 +
                        (parseInt(parts[1], 10) || 0) * 60 +
                        (parseInt(parts[2], 10) || 0);
                }
                else if (parts.length === 2) {
                    serverElapsedSec = (parseInt(parts[0], 10) || 0) * 60 +
                        (parseInt(parts[1], 10) || 0);
                }
                else if (parts.length === 1) {
                    const minutesVal = parseFloat(parts[0]);
                    if (!isNaN(minutesVal)) {
                        serverElapsedSec = Math.round(minutesVal * 60);
                    }
                }
                const durationSec = parseInt(this.timerVal, 10) * 60;
                if (!isNaN(serverElapsedSec) && serverElapsedSec >= durationSec) {
                    this.destroy$.next();
                    this.chargeStoppedAutomatically = true;
                    this.startBlink = false;
                    this.chargeStatusText = _constants__WEBPACK_IMPORTED_MODULE_2__.CHARGE_STATUS_TYPES.TIMEOUT_ERR;
        ...
        this.startChargeReq = this.chargeReq.postRequestDetails(_constants__WEBPACK_IMPORTED_MODULE_2__.RELATIVE_URLS.CHARGING_STATUS, details).subscribe((res) => {
            if (res[0].status.toUpperCase() === 'REQUESTED') {
                ...
                if (this.timeout == null || this.timeout == undefined) {
                    ...
                    }, 60000); // 60 seconds for device to accept the start command
                }
                if (this.isTimedout) {
                    this.timeout = null;
                    if (this.startChargeReq) {
...
ChargeStartStopPage = (0,tslib__WEBPACK_IMPORTED_MODULE_8__.__decorate)([
    (0,_angular_core__WEBPACK_IMPORTED_MODULE_18__.Component)({
        selector: 'app-charge-start-stop',
        template: _home_snehakolimi_workspaces_chargemanpro_node_modules_ngtools_webpack_src_loaders_direct_resource_js_charge_start_stop_page_html__WEBPACK_IMPORTED_MODULE_0__["default"],
